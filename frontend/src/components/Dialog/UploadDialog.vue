<template>
        ...
        <v-card>
          ...
          <v-card-actions>
            <v-btn
              ...
              variant="text"
              @click="(postInfo = false), (upload = true)"
            >
              Back
            </v-btn>
            <v-btn  variant="text" @click="postInfo = false">
              Cancel
            </v-btn>
            <v-btn  variant="text" @click="postVideo()">
              Post
            </v-btn>
          </v-card-actions>
        </v-card>
  ...
  export default {
    data() {
      return {
        ...
        titleRule:[
                      ...
                      v => (v && v.length >= 3 && v.length <= 225) || 'Title must be between 3 and 225 characters'
                    ],
        descriptionRule:[
                      v => !!v || 'Description is required',
                      v => (v && v.length >= 3 && v.length <= 225) || 'Description must be between 10 and 225 characters'
                    ],
        thumbnailRule:[
    ...
    methods: {
      ...
      postVideo(){
        if (this.title && this.description && this.thumbnail && this.privacy && this.category) {
            this.postInfo = false;
            let video = {
              title: this.title,
